...
contests_info_dict_sorted = {contest: {name: st_points for name, st_points in sorted(student_info.items(), key=lambda kvp: (-kvp[1], sum(map(ord, kvp[0][0].lower()))))}\
...
individual_standings_sorted = {name: points for name, points in sorted(individual_standings.items(), key=lambda kvp: (-kvp[1], sum(map(ord, kvp[0][0]))))}
